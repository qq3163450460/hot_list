...
本项目及其全部文档仅供个人学习、技术研究和参考测试使用，请勿用于商业用途或其他正式生产环境。

- 本项目聚合展示的数据均来自各平台公开的热榜信息，相关内容的版权归原作者及各平台所有。
- 本项目与抖音、微博、哔哩哔哩、知乎、今日头条等平台无任何隶属或合作关系，不代表其官方立场。
- 使用本项目时应遵守目标平台的服务条款、频率限制要求及所在地区适用法律法规；因使用或滥用本项目产生的任何直接或间接损失与法律责任，由使用者自行承担。
- 本项目按"现状"提供，不作任何明示或默示的可用性、准确性保证；平台接口变化可能导致功能随时失效。

如不同意上述声明，请立即停止使用并删除本项目。
